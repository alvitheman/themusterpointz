...
const ZTYPES={
 ...
 Spitter:  {hp:130,atk:50,agi:25,interval:1.0,meleeRange:1,xp:50,energy:5,bld:.15,
  spit:true,spitRange:7,spitCd:1.0,spitBurnDps:10,spitBurnT:5},
 // ===== F5: ELITE (§22.2) =====
...
// Boot masuk MAIN MENU — bangun scene, tampilkan overlay, jangan auto-start game.
S.state='menu';
S.paused=true;
// F12.4e: 3D menuScene TIDAK dibangun lagi — menu = foto mainmenu.jpg + efek CSS.
// buildMenuScene() masih ada di code (bisa diaktifkan kembali kalau perlu),
// tapi tidak dipanggil. Menu sekarang overhead-nya ~0 GPU (canvas idle).
showEl('menuOverlay',true);
setGameHudVisible(false); // F12.4b: sembunyikan semua HUD gameplay di menu
{const r=$('rail');if(r){
...
_floatPoolInit(); // F12.13-perf: pre-create 60 float div
loop();